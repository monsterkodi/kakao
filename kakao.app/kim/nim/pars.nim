...
proc parseCallArgList(p:Parser) : seq[Node] = 
    ...
    # echo "parseCallArgList"
    var list : seq[Node]
    let line = p.current().line
    var expr : Node = p.expression(-1)
    while expr != nil:
        list.add expr        
        if p.current().line != line:
            # echo "parseCallArgList linebreak"
            break
        p.swallow ◆comma
        expr = p.expression(-1)
    list
...
proc switchCase(p: Parser, baseIndent: int): Node =
    ...
    # echo &"switchCase case_when {case_when}"
    ...
    # echo &"switchCase of {case_when} ➜ {case_then}"
...
proc lImplicitCall(p: Parser, callee: Node): Node =
    ...
    const notimplicit = {
        ...
        ◆test, ◆func,
        ◆then, ◆else,
        ◆or, ◆and, ◆divide, ◆multiply, ◆equal,
        ◆string_start, ◆comment_start, 
        ◆paren_open, ◆paren_close, ◆square_close, ◆bracket_close, ◆stripol_end
        }
    if p.implicit == false and callee.token.line==token.line and token.tok notin notimplicit:
        # echo &"implicit? {token} {callee.token.tok}"
        if callee.token.tok in {◆name} and not p.isTokenAhead(◆func):
            # echo &"----------------------------- implicit call! {callee} {callee.token}"

            p.implicit = true
            let args = p.parseCallArgList()
            p.implicit = false
            
            return  Node(token:callee.token, kind:●call, callee:callee, callargs:args)
    callee
...
proc parseParenList(p: Parser): seq[Node] =
    ...
    p.swallowError(◆paren_close, "Missing closing parenthesis for call arguments")
...
proc lSymbolList(p: Parser, left: Node) : Node =
    ...
    if p.peek(1).tok in {◆dot, ◆square_open, ◆comma}:
        # echo &"+++++++++++++++++++++++ implicit call {left} {p.current()}"
        return  p.lImplicitCall left

    case left.kind :
        of ●list:
            # echo &"lSymbolList ●list: {left} {p.current()}"
        ...
        of ●literal:
            if left.token.tok != ◆name:
                return  
            # echo &"lSymbolList ●literal: {left} {p.current()}"
            var list_values : seq[Node]
...
proc lFunc(p: Parser, left: Node): Node =
    ...
    if left.kind == ●operation:
        ...
        if left.operand_left.token.col == 0 :
            # echo &"lFunc OP= {left} at line start"
            var left = left
        ...
        for i,a in sig_args.list_values:
            if a.kind == ●operation and a.token.tok == ◆assign:
                # echo &"lFunc list -> >{a.operand_left} = {a.operand_right}< -> ●var"
                let vartoken = Token(tok:◆val, line:a.operand_left.token.line,col:a.operand_left.token.col)
                ...
                # echo &"lFunc list -> |>{a.token.tok}<| -> ●var"
                let vartoken = Token(tok:◆val, line:a.token.line,col:a.token.col)
...
proc rParenExpr(p: Parser): Node =
    ...
    let token = p.current() # (
    let expr = p.parseParenList()
    Node(token:token, kind:●list, list_values:expr)
...
proc expression(p: Parser, precedenceRight = 0): Node =
    ...
    if node.kind == ●list and node.list_values[0].token.tok == ◆name:
        # echo &"expression node == ●list, implicit call! {node}"
        node = Node(token:node.token, kind:●call, callee:node.list_values[0], callargs:node.list_values[1..^1])
...
proc setup(p: Parser) =
    ...
    p.pratt ◆val,               lVarList,          rVar,          100
    p.pratt ◆var,               lVarList,          rVar,          100
