...
suite "rndr":
    ...
    #     t "◆seq[string] files = @[]"                 , "files:var seq[string]=@[]" 
    ...
    # ▸ func
        ...
        # t "l.map(r ◇ tuple ➜ string -> r.path)"    , "l.map(proc (r:tuple) : string = r.path)"
        # 
        # t "f = i ◇ int ->"                         , "proc f(i:int) ="
        # t "f = a ◇ int b ◇ int ->"                 , "proc f(a:int, b:int) ="
        # t "f = a ◇ int = 1 ->"                     , "proc f(a:int=1) ="
...
b = false"""
    ...
    test "strings":
        ...
        t "s = \"\"\"\nl1 #{1+2}\nl2 #{2-3}\"\"\"" , "s = &\"\"\"\nl1 {(1 + 2)}\nl2 {(2 - 3)}\"\"\""
        t "s = \"#{o}\""                           , "s = &\"{o}\""
        # t "let cmd = \"nim c --outDir:#{outdir} --stackTrace:on --lineTrace:on #{file}\"", 
