...
proc lImplicitCall(p: Parser, callee: Node): Node =
    ...
    if p.implicit == false and callee.token.line==token.line and token.tok notin notimplicit:
        ...
        if callee.token.tok in {◆name} and not p.isTokenAhead(◆func) and not p.isTokenAhead(◆assign):
...
proc parseParenList(p: Parser): seq[Node] =
    ...
    if args.len == 1 and args[0].kind == ●list:
        # echo "convert single (list) to seq"
...
proc lSymbolList(p: Parser, left: Node) : Node =
    ...
    # if left.kind in {●list, ●literal} and p.peek(1).tok in {◆dot, ◆comma, ◆square_open, ◆paren_open}
    if left.kind in {●list, ●literal} and p.peek(1).tok in {◆dot, ◆comma, ◆square_open, ◆paren_open}:
        # echo &"+++++++++++++++++++++++ implicit call {left} {p.current()}"
        if left.kind == ●literal and left.token.tok != ◆name:
            return  
...
proc expression(p: Parser, precedenceRight = 0): Node =
    ...
    if node.kind == ●list and node.list_values[0].token.tok == ◆name and not p.isTokenAhead(◆assign):
