...
proc lImplicitCall(p: Parser, callee: Node): Node =
    ...
    Node(token:callee.token, kind:●call, callee:callee, callargs:args)
...
proc lSingleArg(p: Parser, left: Node) : Node =
...
proc expression(p: Parser, precedenceRight = 0): Node =
    ...
    while true:
        ...
        if precedenceRight >= precedence:
            ...
            if p.implicit == false and node.token.line==token.line and token.tok notin notimplicit:
                # echo &"implicit? {token} {node.token.tok}"
                if node.token.tok in {◆name} :
                    # echo &"implicit call! {node} {node.token} {token} {precedence} {precedenceRight}"
