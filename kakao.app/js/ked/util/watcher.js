...
watcher = (function ()
{
    ...
    watcher["watch"] = async function (path, opt)
    {
        ...
        else
        {
            ...
            {
                console.log(`watcher ● ${_k_.g4(path)}`)
                w = fs.watch(path)
        ...
        if (_k_.empty(watcher.watchers[dir]))
        {
            ...
            console.log(`watcher ○ ${_k_.y6(dir)}`)
            w = fs.watch(dir)
            w.on('error',(function (err)
            {
